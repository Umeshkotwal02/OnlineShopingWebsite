const headerCatData = [
  {
    title: "Sale",
    dropdown: [
      {
        category: "Style",
        items: [
          { name: "Wedding Lehenga", link: "/products/wedding-lehenga" },
          { name: "Bridal Lehenga", link: "/products/bridal-lehenga" },
          { name: "Designer Lehenga", link: "/products/designer-lehenga" },
          { name: "Contemporary Lehenga", link: "/products/contemporary-lehenga" },
          { name: "Haldi Lehenga", link: "/products/haldi-lehenga" },
          { name: "See All", link: "/products/lehenga-style" },
        ],
      },
      {
        category: "Fabric",
        items: [
          { name: "Pure Silk Lehenga", link: "/products/pure-silk-lehenga" },
          { name: "Net Lehenga", link: "/products/net-lehenga" },
          { name: "Georgette Lehenga", link: "/products/georgette-lehenga" },
          { name: "Velvet Lehenga", link: "/products/velvet-lehenga" },
          { name: "Art Silk Lehenga", link: "/products/art-silk-lehenga" },
          { name: "See All", link: "/products/lehenga-fabric" },
        ],
      },
      {
        category: "Color",
        items: [
          { name: "Red Lehenga", link: "/products/red-lehenga" },
          { name: "Maroon Lehenga", link: "/products/maroon-lehenga" },
          { name: "Green Lehenga", link: "/products/green-lehenga" },
          { name: "Blue Lehenga", link: "/products/blue-lehenga" },
          { name: "Black Lehenga", link: "/products/black-lehenga" },
          { name: "See All", link: "/products/lehenga-color" },
        ],
      },
    ],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "New Arrivals",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "HALF SAREE",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "FAShiON SAREE",
    dropdown: [
      {
        category: "Style",
        items: [
          { name: "Wedding Lehenga", link: "/products/wedding-lehenga" },
          { name: "Bridal Lehenga", link: "/products/bridal-lehenga" },
          { name: "Designer Lehenga", link: "/products/designer-lehenga" },
          { name: "Contemporary Lehenga", link: "/products/contemporary-lehenga" },
          { name: "Haldi Lehenga", link: "/products/haldi-lehenga" },
          { name: "See All", link: "/products/lehenga-style" },
        ],
      },
      {
        category: "Fabric",
        items: [
          { name: "Pure Silk Lehenga", link: "/products/pure-silk-lehenga" },
          { name: "Net Lehenga", link: "/products/net-lehenga" },
          { name: "Georgette Lehenga", link: "/products/georgette-lehenga" },
          { name: "Velvet Lehenga", link: "/products/velvet-lehenga" },
          { name: "Art Silk Lehenga", link: "/products/art-silk-lehenga" },
          { name: "See All", link: "/products/lehenga-fabric" },
        ],
      },
      {
        category: "Color",
        items: [
          { name: "Red Lehenga", link: "/products/red-lehenga" },
          { name: "Maroon Lehenga", link: "/products/maroon-lehenga" },
          { name: "Green Lehenga", link: "/products/green-lehenga" },
          { name: "Blue Lehenga", link: "/products/blue-lehenga" },
          { name: "Black Lehenga", link: "/products/black-lehenga" },
          { name: "See All", link: "/products/lehenga-color" },
        ],
      },
    ],
    image: "https://kapoorlehengasaree.com/admin/public/category1/75026.jpeg",
  },
  {
    title: "Lahenga",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "Gown",
    dropdown: [
      {
        category: "Style",
        items: [
          { name: "Lehenga Style Sarees", link: "/products/lehenga-style-sarees" },
          { name: "Embroidered Sarees", link: "/products/embroidered-sarees" },
          { name: "Traditional Sarees", link: "/products/traditional-sarees" },
          { name: "Classic Sarees", link: "/products/classic-sarees" },
          { name: "Printed Sarees", link: "/products/printed-sarees" },
          { name: "See All", link: "/products/saree-style" },
        ],
      },
      {
        category: "Fabric",
        items: [
          { name: "Georgette Sarees", link: "/products/georgette-sarees" },
          { name: "Net Sarees", link: "/products/net-sarees" },
          { name: "Banarasi Sarees", link: "/products/banarasi-sarees" },
          { name: "Chiffon Sarees", link: "/products/chiffon-sarees" },
          { name: "Art Silk Sarees", link: "/products/art-silk-sarees" },
          { name: "See All", link: "/products/saree-fabric" },
        ],
      },
    ],
    image: "https://kapoorlehengasaree.com/admin/public/category1/75026.jpeg",
  },

  {
    title: "Wedding",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "Celebrity Outfits",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "OCCASIONS",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "Engagement",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "Reception",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
  {
    title: "OTHERs",
    dropdown: [],
    image: "https://kapoorlehengasaree.com/admin/public/category1/4231.jpeg",
  },
];


// Product Page:


const productsConst = [
  

]

// Product Details Page




export { headerCatData };